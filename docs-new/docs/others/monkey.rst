...
首先配置monkey，使用:c:struct:`lv_monkey_config_t`定义配置结构，设置 ``type``（查看 `input devices</overview/indev>`__以获取支持的类型），然后设置 ``period_range``和 ``input_range``的范围，monkey会在此范围内随机时间输出随机操作。调用:cpp:func:`lv_monkey_create` 创建monkey。最后调用:cpp:expr:`lv_monkey_set_enable(monkey, true)`启用monkey。
