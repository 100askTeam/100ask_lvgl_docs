...
例如，如果你在主页面中创建了一个按钮对象。当你点击该按钮部件时，你希望它打开一个新页面，可以使用 :cpp:expr:`lv_menu_set_load_page_event(menu, btn, new page)`。
...
-  :cpp:enumerator:`LV_EVENT_VALUE_CHANGED` 切换到新页面显示时发送。
   ...
   -  :cpp:expr:`lv_menu_get_cur_main_page(menu)` 会返回一个指向当前显示在主容器中的菜单页面的指针。
   -  :cpp:expr:`lv_menu_get_cur_sidebar_page(menu)` 会返回一个指向当前显示在侧边栏容器中的菜单页面的指针。
