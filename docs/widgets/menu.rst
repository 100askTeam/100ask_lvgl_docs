...
lv_menu_get_cur_main_page（menu） 返回指向菜单页的指针 当前显示在 main 中。

lv_menu_get_cur_sidebar_page（menu） 返回指向菜单的指针 当前显示在边栏中的页面。

当标题中的后退按钮来自任一时发送 单击主栏或侧边栏。已启用 按钮，以便您可以将事件添加到菜单本身。

lv_menu_back_button_is_root（菜单，按钮）检查按钮是否为根 “后退”按钮
